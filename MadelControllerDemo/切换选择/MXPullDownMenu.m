...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color frame:(CGRect)selfFrame backGroundViewInView:(UIView *)view{
    ...
    if (self) {
        ...
        bottomLine.backgroundColor=[UIColor colorWithRed:188/255.0 green:186/255.0 blue:193/255.0 alpha:1];
        ...
        // 创建背景
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height, self.frame.size.width, [[UIScreen mainScreen] bounds].size.height-(self.frame.origin.y+self.frame.size.height))];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete{
    if (show) {
        ...
        CGFloat tableViewHeight = ([tableView numberOfRowsInSection:0] > ([[UIScreen mainScreen]bounds].size.height-(self.frame.origin.y+self.frame.size.height))/tableView.rowHeight) ? (([[UIScreen mainScreen]bounds].size.height-(self.frame.origin.y+self.frame.size.height))) : ([tableView numberOfRowsInSection:0] * tableView.rowHeight);
        [UIView animateWithDuration:0.2 animations:^{
